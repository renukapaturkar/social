...
import { getFollowing } from './usersSlice'
  ...
  const renderFollowings =
    followings.length === 0 ? (
      <div className="text-center">
        <p className="p-2">Not following anyone yet</p>
      </div>
    ) : (
        ...
        followings?.map((user) => (
          <div className="flex space-evenly p-4 ">
            {user.profilePicture ? (
              ...
              <div
                className={`flex text-xl capitalize text-white items-center justify-center rounded-full w-10 h-10 md:w-10 md:h-10 bg-purple-600`}
              >
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="bg-white mt-1 p-3 md:p-3 md:p-0 w-full md:w-1/2 rounded">
        ...
        {status === 'success' && renderFollowings}
      </div>
    </div>
  )
}
